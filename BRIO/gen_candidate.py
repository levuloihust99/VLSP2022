...
def generate_summaries_abmusu(args):
    device = f"cuda:{args.gpuid}"
    mname = "/media/lvloi/projects/vlsp-2022/seq2seq/pretrained/checkpoints/checkpoint-231"
    model = T5ForConditionalGeneration.from_pretrained(mname).to(device)
    tokenizer = AutoTokenizer.from_pretrained("VietAI/vit5-base-vietnews-summarization")
    model.eval()
    data = []
    with open(args.src_dir, "r") as reader:
        for line in reader:
            data.append(json.loads(line.strip()))
    
    writer = open(args.tgt_dir, "w")
    for item in tqdm(data):
        inputs = tokenizer(item['document'], return_tensors='pt')
...
if __name__ ==  "__main__":
    ...
    if args.dataset == "cnndm":
        ...
        generate_summaries_abmusu(args)
