...
from .utils import convert_dictconfig_to_dict
...
class BertExtractiveTrainer(object):
    ...
    def save_checkpoint(self, step):
        cp_name = "bert_ext.{}".format(step + 1)
        model_state_dict = self.model.state_dict()
        optimizer_state_dict = self.optimizer.state_dict()
        scheduler_state_dict = self.scheduler.state_dict()
        training_args = convert_dictconfig_to_dict(self.cfg)
        ...
        for step in range(self.saved_state['trained_steps'], self.cfg.total_updates):
            ...
            if (step + 1) % self.cfg.logging_steps == 0:
                logger.info("Step {}/{}  |  Loss: {} | Time elapsed: {}".
                    format(step + 1, self.cfg.total_updates, per_update_loss, time.perf_counter() - self._mark_time))
                self._mark_time = time.perf_counter()
